...
class Pmt {
  initialize(K,H,r) {
    K=K||0;
    this.policy.${key}||(H=DTN(this.Zb),this.audioTrack,H.videoTrack);
...
test('patches the playback initialize structurally and detects the current minified key', () => {
  ...
  expect(result.source).toContain('globalThis.__PEARD_FORCE_DIRECT_HQ__===true');
...
test('injects before the original initialize body', () => {
  const result = patchPlayerScript(makeSource());
  const injection = result.source.indexOf('globalThis.__PEARD_FORCE_DIRECT_HQ__');
