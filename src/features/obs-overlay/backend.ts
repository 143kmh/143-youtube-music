...
export const createObsOverlayBackend = (port = PREFERRED_PORT) =>
  createBackend<ObsOverlayBackendState>({
    server: null,
    ...
    async start({ ipc }) {
      ...
      // OBS stores this URL in its scene. Never silently move to another port:
      // keep retrying the same address when a previous process is still exiting.
      let lastErrorCode = '';
      const listen = () => {
        if (!this.running) return;
        ...
        server.once('error', (error: NodeJS.ErrnoException) => {
          if (lastErrorCode !== error.code) {
            console.warn(
              '[143 Music] OBS overlay waiting for its fixed address:',
              error.code,
            );
...
export default createObsOverlayBackend();
